  ...
  <div class="space-y-8">
    ...
    <form @submit.prevent="publish" class="space-y-6" novalidate>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <div class="space-y-2"><label class="field-label">Location</label><input v-model="form.location" type="text" class="sci-input" /></div>
      </div>

      <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div class="space-y-2"><label class="field-label">Date</label><input v-model="form.date" type="text" class="sci-input" /></div>
        <div class="space-y-2"><label class="field-label">Mood</label><input v-model="form.mood" type="text" class="sci-input" /></div>
      </div>

      <div class="space-y-2"><label class="field-label">Description</label><textarea v-model="form.description" class="sci-input resize-none" rows="2" /></div>

      <div class="flex items-center justify-between pt-2">
        <p class="text-[10px] font-mono text-slate-700 tracking-wider">// PHYSICAL ARCHIVE ENTRY</p>
    ...
    <div>
      ...
      <p class="field-label mb-4">Existing Memories · {{ entries.length }}</p>
      ...
      <div v-else-if="entries.length === 0" class="card-glass p-6 text-center">
        <p class="font-mono text-xs tracking-wider text-slate-600">// NO MEMORIES YET</p>
      </div>
      ...
      <div v-else class="space-y-2">
        <div v-for="e in entries" :key="e.id" class="card-glass p-4 flex items-center gap-4 group">
          <div class="w-12 h-12 shrink-0 rounded overflow-hidden bg-slate-950 border border-slate-800/60">
            <img v-if="e.photoUrl" :src="e.photoUrl" class="w-full h-full object-cover" />
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="font-mono text-[10px] tracking-wider text-slate-500">{{ e.location }} · {{ e.date }}</p>
          </div>
          <span class="font-mono text-[9px] tracking-wider text-slate-500 shrink-0">{{ e.mood }}</span>
...
import { adminUpload, adminDelete, fetchSportMemories } from '../api/admin'

const props = defineProps({ token: { type: String, required: true } })
const emit = defineEmits(['toast'])

const form = reactive({ title: '', location: '', date: '', description: '', mood: '' })
...
async function publish() {
  ...
  try {
    await adminUpload('sports', file.value, { ...form }, props.token.trim())
    emit('toast', { ok: true, message: 'SPORTS MEMORY SAVED' })
    Object.assign(form, { title: '', location: '', date: '', description: '', mood: '' })
...
async function loadEntries() {
  loading.value = true
  try { const data = await fetchSportMemories(); entries.value = Array.isArray(data) ? data : [] } catch (err) { console.error(err) }
...
async function remove(id) {
  ...
  deleting.value = id
  try { await adminDelete('sports', id, props.token.trim()); entries.value = entries.value.filter(e => e.id !== id); emit('toast', { ok: true, message: 'MEMORY DELETED' }) }
